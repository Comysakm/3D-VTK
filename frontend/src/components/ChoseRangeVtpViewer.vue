<!--
  代码功能说明：
  这是一个基于 Vue 3 和 VTK.js 的三维可视化应用，用于从后端获取 VTP 格式的点云数据，并以立方体形式渲染。用户可通过控制面板设置标量过滤范围（最小值、最大值）和立方体大小，点云根据标量值应用蓝-绿-红渐变颜色映射，展示动态三维可视化效果。使用 VTK.js 的 Glyph3DMapper 实现立方体绘制，结合颜色映射和用户交互功能。

  主要功能：
  1. 初始化 VTK.js 渲染器，设置全屏渲染窗口和深灰色背景。
  2. 通过控制面板输入最小值、最大值和立方体大小，请求后端生成过滤后的 VTP 点云数据。
  3. 解压并解析 VTP 文件，提取点云数据和标量值。
  4. 使用 vtkGlyph3DMapper 将点云渲染为立方体，立方体大小由用户输入控制，颜色根据标量值映射为蓝-绿-红渐变。
  5. 显示加载进度、文件信息（大小和点数）以及错误提示。
  6. 记录请求、解压、解析和渲染的耗时日志，自动保存到后端。
  7. 在组件卸载时清理 VTK 资源以防止内存泄漏。

  代码结构：
  - **模板部分**：包含全屏渲染容器和控制面板，用于用户交互和 VTK.js 渲染。
  - **脚本部分**：使用 Vue 3 的组合式 API（<script setup>），实现数据请求、解压、解析、渲染和日志记录。
  - **样式部分**：定义全屏布局和控制面板样式，确保渲染窗口和交互控件美观实用。

  注意事项：
  - 需确保后端服务（http://localhost:5000）正常运行以提供 VTP 数据和保存日志。
  - 点云数据量可能较大，渲染性能受硬件限制，建议根据设备性能优化 VTP 数据规模。
  - 颜色映射基于点云标量范围，标量缺失时使用默认颜色。
-->
